...
// يتم تجاهل استيراد useEffect وFirebase بشكل متعمد هنا للتشغيل السريع في بيئات التطوير التي لا تحتوي على إعدادات Firebase مسبقة.

function App() {
  // 1. تعريف أسئلة المسابقة
  const questions = [
    {
  ...
  // 2. إدارة حالة المكون (State Management)
  ...
  // متغير وهمي (Mock) لمعرّف المستخدم
  ...
  // 4. دالة التعامل مع اختيار الإجابة
  const handleAnswerClick = (option) => {
    if (selectedAnswer !== null) return; // منع تغيير الإجابة بعد الاختيار
    ...
    if (isCorrect) {
      ...
      setFeedback(`إجابة خاطئة. الإجابة الصحيحة هي: ${questions[currentQuestionIndex].correctAnswer}`);
    }
  };

  // 5. دالة الانتقال للسؤال التالي
  const handleNextQuestion = () => {
    if (selectedAnswer === null) return; // لا تسمح بالانتقال دون اختيار إجابة
  ...
  // 6. دالة إعادة تشغيل المسابقة
  const handleRestartQuiz = () => {
    // إعادة تعيين حالة quizStarted إلى false لعرض شاشة البداية أولاً
    setQuizStarted(false); 
    // إعادة تعيين جميع العدادات والحالات
    handleStartQuiz();
  };
  ...
  return (
    <div
      ...
      dir="rtl" // لضمان اتجاه النص العربي بالكامل
    >
      <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-2xl text-center flex flex-col items-center border border-indigo-300">
        ...
        {userId && (
          <p className="text-sm text-gray-500 mb-4">
            معرّف المستخدم: <span className="font-semibold text-gray-700 break-words">{userId}</span>
          </p>
        )}
        ...
        {!quizStarted ? (
          <div className="flex flex-col items-center">
            <p className="text-2xl text-gray-700 mb-8 max-w-prose">
              اختبر معلوماتك في الشعر العربي وأبحر في عوالم الكلمة الساحرة.
            </p>
            <button
        ...
        showResults ? (
          <div className="w-full">
            <h2 className="text-4xl font-bold text-indigo-800 mb-6">النتائج النهائية</h2>
            <p className="text-3xl text-gray-700 mb-8">
              لقد أحرزت <span className="font-extrabold text-emerald-600">{score}</span> من أصل <span className="font-extrabold text-indigo-600">{questions.length}</span> نقاط!
            </p>
            <p className="text-xl text-gray-600 mb-10">
              {score === questions.length ? (
                'أنت بالفعل ملك اللغة العربية!🏆'
              ) : score >= questions.length / 2 ? (
                'أحسنت! أداء جيد جداً، استمر في المراجعة.✨'
              ) : (
                'لا بأس، لديك فرصة أخرى لتصبح ملك اللغة العربية! 📚'
              )}
            </p>
            <button
          ...
          <div className="w-full text-right flex flex-col items-center">
            ...
            <button
              ...
              `}
            >
              {currentQuestionIndex === questions.length - 1 ? 'إنهاء المسابقة' : 'السؤال التالي'}
